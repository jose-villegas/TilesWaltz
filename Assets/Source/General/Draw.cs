namespace TilesWalk.General
{
	...
	public static class Draw
	{
		...
		/// </summary>
		public static void Orbit3D(Vector3 center, float eccentricity, float semiMajorAxis, Vector3 normal,
			Vector3 forward)
		{
			Prepare3D();
		...
		/// </summary>
		public static void Orbit3DApses(Vector3 center, float periapsis, float apoapsis, Vector3 normal,
			Vector3 forward)
		{
			float a = (periapsis + apoapsis) / 2;
			float e = (apoapsis - periapsis) / (apoapsis + periapsis);
		...
		public static void Sphere(Vector3 center, float radius, int verticalSegments = 1, int radialSegments = 2)
		{
			if (radialSegments > 2)
			{
				...
				{
					Vector3 normal = new Vector3(Mathf.Sin((i * Mathf.PI) / radialSegments), 0,
						Mathf.Cos((i * Mathf.PI) / radialSegments));
